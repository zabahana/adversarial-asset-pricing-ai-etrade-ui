...
class DataExplorer:
    ...
    def visualize_price_trends(self, df):
        ...
        for idx, symbol in enumerate(symbols):
            ...
            if 'sma_20' in symbol_data.columns and symbol_data['sma_20'].notna().any():
    ...
    def visualize_technical_indicators(self, df):
        ...
        for symbol in symbols[:2]:  # First 2 symbols
            ...
            # Price Changes
            ax2 = axes[1]
            clean_changes = symbol_data['price_change_1d'].dropna()
            if len(clean_changes) > 0:
                ax2.bar(symbol_data['timestamp'][:len(clean_changes)], clean_changes, 
            ...
            # Volatility
            ax3 = axes[2]
            clean_vol = symbol_data['volatility_10d'].dropna()
            if len(clean_vol) > 0:
                ax3.plot(symbol_data['timestamp'][:len(clean_vol)], clean_vol, 
                        color='red', linewidth=2)
                ax3.fill_between(symbol_data['timestamp'][:len(clean_vol)], clean_vol, 
                               alpha=0.3, color='red')
    ...
    def distribution_analysis(self, df):
        ...
        numeric_cols = ['close_price', 'price_change_1d', 'sma_20', 'volatility_10d']
        # Filter to only columns that exist and have data
        numeric_cols = [col for col in numeric_cols if col in df.columns and df[col].notna().any()]
        ...
        for idx, col in enumerate(numeric_cols):
            ...
            if len(clean_data) > 0:
                clean_data.hist(bins=30, ax=ax, edgecolor='black', alpha=0.7)
        ...
        # Hide empty subplots
        for idx in range(len(numeric_cols), n_rows * 2):
